...
def rearrange_micro_batches(batch: TensorDict, max_token_len, dp_group=None, non_tensor_batch=None):
    ...
    if non_tensor_batch is not None:
        for k in non_tensor_batch.keys():
            non_tensor_batch[k] = non_tensor_batch[k].tolist()
    ...
    # assert num_micro_batches <= len(seq_len_effective), f'{num_micro_batches=} is greater than {len(seq_len_effective)=}'
    
    if num_micro_batches > len(seq_len_effective):
        num_to_add = num_micro_batches - len(seq_len_effective)
        indices_to_duplicate = torch.randint(0, len(seq_len_effective), (num_to_add,)).tolist()
        duplicated_samples = []
        for idx in indices_to_duplicate:
            duplicated_samples.append(batch[idx:idx + 1])
        batch = torch.cat([batch] + duplicated_samples, dim=0)
        if non_tensor_batch is not None:
            for k in non_tensor_batch.keys():
                duplicated_items = [non_tensor_batch[k][idx] for idx in indices_to_duplicate]
                non_tensor_batch[k].extend(duplicated_items)
        seq_len_effective.extend([seq_len_effective[idx] for idx in indices_to_duplicate])
    ...
    for partition in micro_bsz_idx:
        curr_micro_batch = []
        if non_tensor_batch is not None:
            curr_micro_non_tensor_batch = {k: [] for k in non_tensor_batch.keys()}
        for idx in partition:
            curr_micro_batch.append(batch[idx:idx + 1])
            if non_tensor_batch is not None:
                for k in non_tensor_batch.keys():
                    curr_micro_non_tensor_batch[k].append(non_tensor_batch[k][idx])
...
if __name__ == '__main__':
    ...
    a = [1,2,3]
    num = 4
    micro = get_seqlen_balanced_partitions(a, num, equal_size=False)
    print(micro)