...
QWEN_SYSTEM_PREFIX = "system\nYou are Qwen, created by Alibaba Cloud. You are a helpful assistant.\nuser\n"
QWEN3_SYSTEM_PREFIX = "user\n"
THINK_SYSTEM_PREFIX = "system\nYou are a helpful assistant. To answer a query from the user, please first think about the reasoning process in the mind and then output your answer. The reasoning process is enclosed within <think> </think> tags, i.e., <think> reasoning process here </think> answer here.\nuser\n"
THINK_ANSWER_SYSTEM_PREFIX = "system\nYou are a helpful assistant. Please first thinks about the reasoning process in the mind and then provides the user with the answer. The reasoning process is enclosed within <think> </think> and answer is enclosed within <answer> </answer> tags, respectively, i.e., <think> reasoning process here </think> <answer> answer here </answer>.\nuser\n"
LLAMA_THINK_SYSTEM_PREFIX = "system\n\nYou are a helpful assistant. To answer a query from the user, please first think about the reasoning process in the mind and then output your answer. The reasoning process is enclosed within <think> </think> tags, i.e., <think> reasoning process here </think> answer here.user\n\n"
SYSTEM_PREFIX = "system\nYou are a helpful assistant.\nuser\n"
ASSISTANT_PREFIX = "\nassistant\n"
LLAMA_ASSISTANT_PREFIX = "assistant\n\n"

DOMAIN_PREFIX = ["math", "codegen", "logic", "table", "simulation", "stem", "creative"]
...
def sort_and_shuffle_dataset(train_dataset, key='qwen2.5_7b_pass_rate'):
    """
    按照 key 从高到低排序，将 NaN 值样本随机插入
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # NOTE: all_count and mean_info is added by xize
        # advantages.shape: (bs, response_length)
        advantages, returns, all_count, mean_info = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=data.batch['token_level_rewards'],
            response_mask=data.batch['response_mask'],
        ...
        data.batch['returns'] = returns
        # NOTE: added by xize
        data.meta_info["all_1"] = all_count["all_1"]
        data.meta_info["all_neg"] = all_count["all_neg"]
        data.meta_info["all_m05"] = all_count["all_m05"]
        data.meta_info["all_m1"] = all_count["all_m1"]
        data.meta_info["mean_ratio_1"] = mean_info["mean_ratio_1"]
        data.meta_info["mean_ratio_m05"] = mean_info["mean_ratio_m05"]
        data.meta_info["mean_ratio_m1"] = mean_info["mean_ratio_m1"]
        data.meta_info["mean_acc"] = mean_info["mean_acc"]
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        # NOTE: added by xize, 按照某些规则对train_dataset进行重排。下面的shuffle需要设为False.
        if self.config.data.curriculum == "selfpaced":
            print(f"We are using Self-paced curriculum learning")
            self.train_dataset = sort_and_shuffle_dataset(self.train_dataset, key="qwen2.5_7b_pass_rate")
            self.config.data.shuffle = False
        elif self.config.data.curriculum == "vanilla":
            print(f"We are using vanilla curriculum learning")
            self.train_dataset = sort_by_prompt_length_per_category_then_interleave(self.train_dataset)
            self.config.data.shuffle = False
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # if self.use_rm:
            #     # breakpoint()
            #     test_reward_tensor = self.rm_wg.compute_rm_score(test_batch)
            #     test_batch = test_batch.union(test_reward_tensor)
    ...
    def fit(self):
        ...
        os.makedirs(self.config.trainer.default_question_and_response_path, exist_ok=True)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                batch: DataProto = DataProto.from_single_dict(batch_dict)  # batch.batch, batch.non_tensor_batch
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        for i in range(len(batch)):
                            ...
                            prompt_and_response = self.tokenizer.decode(batch.batch[i]['input_ids'], skip_special_tokens=True)
                            if prompt_and_response.startswith(QWEN_SYSTEM_PREFIX):
                                question_and_response = prompt_and_response.split(QWEN_SYSTEM_PREFIX, 1)[-1]
                                question, response = question_and_response.split(ASSISTANT_PREFIX, 1)
                            elif prompt_and_response.startswith(SYSTEM_PREFIX):
                                question_and_response = prompt_and_response.split(SYSTEM_PREFIX, 1)[-1]
                                question, response = question_and_response.split(ASSISTANT_PREFIX, 1)
                            elif prompt_and_response.startswith(THINK_SYSTEM_PREFIX):
                                question_and_response = prompt_and_response.split(THINK_SYSTEM_PREFIX, 1)[-1]
                                question, response = question_and_response.split(ASSISTANT_PREFIX, 1)
                            elif prompt_and_response.startswith(THINK_ANSWER_SYSTEM_PREFIX):
                                question_and_response = prompt_and_response.split(THINK_ANSWER_SYSTEM_PREFIX, 1)[-1]
                                question, response = question_and_response.split(ASSISTANT_PREFIX, 1)
                            elif prompt_and_response.startswith(QWEN3_SYSTEM_PREFIX):
                                question_and_response = prompt_and_response.split(QWEN3_SYSTEM_PREFIX, 1)[-1]
                                question, response = question_and_response.split(ASSISTANT_PREFIX, 1)
                            elif prompt_and_response.startswith(LLAMA_THINK_SYSTEM_PREFIX):
                                question_and_response = prompt_and_response.split(LLAMA_THINK_SYSTEM_PREFIX, 1)[-1]
                                question, response = question_and_response.split(LLAMA_ASSISTANT_PREFIX, 1)
                            reward = batch.batch[i]['token_level_scores'].sum().item()

                            with open(f"{self.config.trainer.default_question_and_response_path}/{domain}.jsonl", "a") as f:
                                record = {
                                    "question": question,
                                    "response": response,
                                    "reward": reward,
