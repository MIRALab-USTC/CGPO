...
def compute_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            # NOTE: added by xize
            ratio_1 = id2score[idx].count(1) / len(id2score[idx])
            ratio_m05 = id2score[idx].count(-0.5) / len(id2score[idx])
            ratio_m1 = id2score[idx].count(-1) / len(id2score[idx])
            ratio_neg = ratio_m05 + ratio_m1
            
            ratio_1_list.append(ratio_1)
            ratio_m05_list.append(ratio_m05)
            ratio_m1_list.append(ratio_m1)
            ratio_neg_list.append(ratio_neg)
            ...
            # NOTE: added by xize
            mean_list.append(id2mean[idx].item())
        for i in range(bsz):
            scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
        scores = scores.unsqueeze(-1) * response_mask

        all_count = {
            "all_1": all_1 / len(id2score),
            "all_neg": all_neg / len(id2score),
            "all_m05": all_m05 / len(id2score),
            "all_m1": all_m1 / len(id2score),
        }
        
        mean_ratio_1 = sum(ratio_1_list) / len(ratio_1_list)
        mean_ratio_m05 = sum(ratio_m05_list) / len(ratio_m05_list)
        mean_ratio_m1 = sum(ratio_m1_list) / len(ratio_m1_list)
        mean_acc = sum(mean_list) / len(mean_list)
        
        mean_info = {
            "mean_ratio_1": mean_ratio_1,
            "mean_ratio_m05": mean_ratio_m05,
            "mean_ratio_m1": mean_ratio_m1,
            "mean_acc": mean_acc,
        }
        
    return scores, scores, all_count, mean_info
